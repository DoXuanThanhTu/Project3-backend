...
exports.MovieFlagService = void 0;
...
class MovieFlagService {
    ...
    async addAdminFlag(data) {
        ...
        return await movie_model_1.MovieModel.findByIdAndUpdate(movieId, {
            $push: { flags: flag },
            $set: { [`flags.${type}`]: true }, // Optional: có thể thêm field boolean riêng
        }, { new: true });
    }
    async removeAdminFlag(movieId, flagType) {
        return await movie_model_1.MovieModel.findByIdAndUpdate(movieId, {
            $pull: {
                flags: {
                    type: flagType,
    ...
    async updateAdminFlag(movieId, flagType, updates) {
    ...
    async calculateTrendingFlags() {
        const yesterday = new Date();
        yesterday.setDate(yesterday.getDate() - 1);
        // Lấy phim có dailyViews tăng đột biến
        const movies = await movie_model_1.MovieModel.aggregate([
            {
                $match: {
        ...
        for (const movie of movies) {
            ...
            endAt.setHours(endAt.getHours() + this.systemConfig.trending.durationHours);
            await this.addSystemFlag({
                movieId: movie._id.toString(),
                type: movie_type_1.MovieFlagType.TRENDING,
                endAt,
                score: movie.trendingScore,
            });
            // Cập nhật thời gian tính trending lần cuối
            await movie_model_1.MovieModel.findByIdAndUpdate(movie._id, {
                lastTrendingUpdate: new Date(),
            });
        }
    }
    async calculateHotFlags() {
        const movies = await movie_model_1.MovieModel.aggregate([
            {
                $match: {
        ...
        for (const movie of movies) {
            ...
            await this.addSystemFlag({
                movieId: movie._id.toString(),
                type: movie_type_1.MovieFlagType.HOT,
                endAt,
                score: movie.hotScore,
            });
        }
    }
    async calculateFeaturedFlags() {
        const movies = await movie_model_1.MovieModel.aggregate([
            {
                $match: {
                    ratingAvg: { $gte: 4.0 },
        ...
        for (const movie of movies) {
            const endAt = new Date();
            endAt.setHours(endAt.getHours() + this.systemConfig.featured.durationHours);
            await this.addSystemFlag({
                movieId: movie._id.toString(),
                type: movie_type_1.MovieFlagType.FEATURED,
                endAt,
                score: movie.featuredScore,
            });
        }
    }
    async addSystemFlag(data) {
        // Xóa flag system cũ cùng loại
        await movie_model_1.MovieModel.findByIdAndUpdate(data.movieId, {
            $pull: {
                flags: {
                    type: data.type,
                    source: "system",
    ...
    async cleanExpiredFlags() {
        const now = new Date();
        await movie_model_1.MovieModel.updateMany({}, {
            $pull: {
                flags: {
    ...
    // ===== QUERY METHODS =====
    async getMoviesWithFlag(flagType, options = {}) {
    ...
    async getActiveFlags(movieId) {
        const movie = await movie_model_1.MovieModel.findById(movieId);
        if (!movie)
            return [];
        const now = new Date();
        return movie.flags.filter((flag) => flag.endAt === null || flag.endAt >= now);
    }
}
exports.MovieFlagService = MovieFlagService;
